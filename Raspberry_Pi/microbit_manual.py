...
class Microbit:
    ...
    def __init__(self, device_addr, adapter_addr=None):
        ...
        self._temp_period = self.ubit.add_characteristic(TEMP_SRV,
                                                         TEMP_PERIOD)
        self.uart_data = self.ubit.add_characteristic(UART_SRV,
		                                               UART_TX)
    @property
    def connected(self):
	...
	def read_uart(self):
		"""
		Reads the text sent from Micro:Bit UART Service
		"""
		text = ''
		text.join(chr(i) for i in self._uart_data.value)		
		return text
    ...
    def subscribe_uart(self, user_callback):
        """
        Execute user_callback on UART data received
        :param user_callback: function to call on new data
        """
        self.uart_data.add_characteristic_cb(user_callback)
        self.uart_data.start_notify()
