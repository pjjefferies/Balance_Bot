...
def main():
	...
	if os.name == 'posix' and os.uname()[1] == 'raspberrypi':
		...
		logger.warning('Test Encoder Sensor with Motor not designed to run on Windows at this time')
		return
	else:
		logger.warning('Test Encoder Sensor with Motor  - OS not identified. Please try on Raspberry Pi')
		return
